...
```{r}
df2 <- df %>%
  dplyr::select(nT, AvgED, MedED, mBS, mCAPE) %>%
  dplyr::mutate(nTs = scale(log(nT)), AvgEDs = scale(log(AvgED)), 
                MedEDs = scale(log(MedED)),
                mBSs = scale(mBS), mCAPEs = scale(mCAPE))
...
out.df <- data.frame(theta, r, pval)

ggplot(out.df, aes(x = theta, y = r)) + 
  geom_line() +
