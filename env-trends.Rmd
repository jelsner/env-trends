...
Get environmental data at 18Z (2p local) on the convective day. Set up a vector of URLs as character strings. Data are not available on the NCDC server after September 30, 2014.
```{r}
df <- BigDayHulls.sfT %>%
...
```{r}
df2 <- df %>%
  dplyr::select(nT, AvgATE, mBS, mCAPE) %>%
  dplyr::mutate(nTs = scale(log(nT)), AvgATEs = scale(log(AvgATE)), mBSs = scale(mBS), mCAPEs = scale(mCAPE))
ggplot(df2, aes(x = AvgATEs, y = nTs, color = mCAPEs)) +
  geom_point() 
```

```{r}
df2 %>%
  group_by(AvgATEs < 0 & nTs > 0) %>%
...
for (k in theta){
  C1 <- cos(k * pi/180)
  C2 <- sin(k * pi/180)
  Tclim <- C1 * scale(log(df$AvgATE)) + C2 * scale(log(df$nT))
...
for (k in theta){
  ...
  ctest = cor.test(Eclim, scale(log(df$AvgATE)))
  r <- c(r, as.numeric(ctest$estimate))
  pval <- c(pval, as.numeric(ctest$p.value))
}
range(r)
...
text(par('usr')[2] - 1.52, 1.17, srt=0, adj = 0, labels = 'Shear', xpd = TRUE, cex=1.3)
text(par('usr')[2] - 0.6, -0.81, srt=0, adj = 0, labels = '', xpd = TRUE, cex=1.3)
text(0,0.55, '0.5', cex=1.4, col=colors()[229])
text(0,1.05, '1.0', cex=1.4, col=colors()[229])
