...
p1 <- df %>%
  filter(Yr < 2014) %>%
  group_by(Yr) %>%
  summarize(AnnualAvg = mean(mHLCY, na.rm = TRUE)) %>%
...
p2 <- df %>%
  filter(Yr < 2014) %>%
  group_by(Yr) %>%
  summarize(AnnualAvg = mean(mCIN, na.rm = TRUE)) %>%
ggplot(., aes(x = Yr, y = -AnnualAvg)) +
  geom_point()  + geom_smooth(method = lm, color = "black", size = .35) +
  scale_y_continuous(limits = c(0, NA)) +
  scale_x_continuous(breaks = seq(1994, 2013, 4)) +
...
p3 <- df %>%
  filter(Yr < 2014) %>%
  group_by(Yr) %>%
  summarize(AnnualAvg = mean(mBS, na.rm = TRUE)) %>%
ggplot(., aes(x = Yr, y = AnnualAvg)) +
  geom_point() + geom_smooth(method = lm, color = "black", size = .35) +
  scale_y_continuous(limits = c(0, NA)) +
  scale_x_continuous(breaks = seq(1994, 2013, 4)) +
...
p4 <- df %>%
  filter(Yr < 2014) %>%
  group_by(Yr) %>%
  summarize(AnnualAvg = mean(mCAPE, na.rm = TRUE)) %>%
ggplot(., aes(x = Yr, y = AnnualAvg)) +
  geom_point() + geom_smooth(method = lm, color = "black", size = .35) +
  scale_y_continuous(limits = c(0, NA)) +
  scale_x_continuous(breaks = seq(1994, 2013, 4)) +
